...
- Загрузки лежат в `backend/uploads/` на диске и отдаются через StaticFiles — на Railway
  без volume теряются при редеплое.
...
- Миграции запускаются в startup-хуке приложения. При масштабировании больше чем в один
  инстанс правильнее вынести их в отдельный release-команду Railway.
