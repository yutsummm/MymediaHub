...
def init_db():
    ...
    if c.fetchone()["count"] == 0:
        ...
        for title, content, status, platforms, tags, days, _views, _reactions, _comments, _shares, tmpl in rows:
            ...
            c.execute(
                ...
                (title, content, status, json.dumps(platforms), json.dumps(tags), scheduled_at, published_at, 0, 0, 0, 0, random.choice([1, 2]), tmpl, created_at),
            )
...
def publish_group_post(gid: int, post_id: int, user_id: int = Depends(get_current_user_id)):
    ...
    # Store vk_post_id and tg_message_ids in DB
...
@app.post("/api/posts/sync-vk-stats")
def sync_vk_stats():
    """Fetch real views/likes/reposts/comments from VK for all published posts."""
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT group_id, access_token FROM vk_settings WHERE id=1")
    vk = c.fetchone()
    ...
    # Also check group-scoped settings
    ...
    for vk_conf in unique_vk:
        ...
        if not posts:
            continue
        # Batch up to 100 post IDs per request (VK limit)
        batch = []
        for p in posts:
            ...
            try:
                ...
                for vp in vk_posts:
                    ...
                    c.execute(
                        ...
                        (views, reactions, comments, shares, datetime.now().strftime("%Y-%m-%dT%H:%M"), vp_id),
                    )
                    total_synced += 1
            except Exception:
                continue
...
@app.get("/api/groups/{gid}/analytics/summary")
...
@app.get("/api/groups/{gid}/analytics/timeline")
...
@app.get("/api/groups/{gid}/analytics/export")
def group_analytics_export(gid: int, start_date: str = Query(...), end_date: str = Query(...), user_id: int = Depends(get_current_user_id)):
    now = datetime.now()
    try:
        dt_start = datetime.strptime(start_date, "%Y-%m-%d")
        dt_end   = datetime.strptime(end_date,   "%Y-%m-%d")
    ...
    for i in range(delta):
        day = dt_start + timedelta(days=i)
        ds  = day.strftime("%Y-%m-%d")
        c.execute("SELECT SUM(views) v, SUM(reactions) r, COUNT(*) p FROM posts WHERE group_id=%s AND published_at LIKE %s", (gid, ds + "%"))
        row = c.fetchone()
        timeline.append({"date": ds, "label": day.strftime("%d.%m"), "views": row["v"] or 0, "reactions": row["r"] or 0, "posts": row["p"] or 0})
...
# ── Group-scoped VK Stats Sync ─────────────────────────────────────────────────

@app.post("/api/groups/{gid}/posts/sync-vk-stats")
        ...
        try:
            ...
            for vp in vk_posts:
                ...
                c.execute(
                    ...
                    (views, reactions, comments, shares, datetime.now().strftime("%Y-%m-%dT%H:%M"), vp_id, gid),
                )
                total_synced += 1
        except Exception:
            continue
